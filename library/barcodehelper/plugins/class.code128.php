...
class Code128
{
	...
	private static function getKey($type, $char)
	{
		switch($type)
		{
			case self::TYPE_A:
	...
	private static function getBar($type, $char)
	{
		$key = self::getKey($type, $char);
		return self::$barMap[($key !== false) ? $key : 0];
	}

	private static function getStartChar($type)
	{
		switch($type)
		{
			case self::TYPE_A:
	...
	{
		if(!isset($ext['type']) || $ext['type'] < 1 || $ext['type'] > 3)
		{
			if(is_numeric($text))
			{
				$type = self::TYPE_C;
		...
		array_unshift($charAry, self::getStartChar($type));
		$checkSumCollector = self::getKey($type, self::getStartChar($type));
		foreach($charAry as $k => $char)
		{
			$code = self::getBar($type, $char);
			$checkSumCollector += self::getKey($type, $char) * $k; // $k will be 0 for our first

...
$.widget( "evol.colorpicker", {
	...
	_create: function() {
		...
		switch(this.element.get(0).tagName){
			case 'INPUT':
				...
				var posrel = '';
				if(this.options.valign_top){
					this.options.valign_top = $(this.element.get(0)).outerHeight() + 1;
					posrel = 'position:relative;';
				}
				e.addClass('colorPicker '+this._id)
					.wrap('<div style="' + posrel + 'width:'+(this.element.width()+32)+'px;'+
	...
	_paletteHTML: function() {		
		...
		return [
			...
			this._isPopup?' style="' + (this.options.valign_top ? ('bottom:' + this.options.valign_top + 'px;') : '') + 'position:absolute"':'','>',
            // palette
